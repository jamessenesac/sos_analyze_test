...
#  log
  ...
  log "// pulpcore task failures"
  log "$GREP_EXTENDED -n 'Traceback|CRITICAL .*task|Exception.*pulpcore' $base_dir/var/log/pulpcore/*.log* 2>/dev/null | head -30"
  log "---"
  log_cmd "$GREP_EXTENDED -n 'Traceback|CRITICAL .*task|Exception.*pulpcore' $base_dir/var/log/pulpcore/*.log* 2>/dev/null | head -30"
  log "---"
  log

  log "// dynflow queue warnings"
  log "$GREP_EXTENDED -n '(lag|executor|crash|timeout|failed|backlog|stuck)' $base_dir/var/log/foreman/dynflow*log* 2>/dev/null | head -30"
  log "---"
  log_cmd "$GREP_EXTENDED -n '(lag|executor|crash|timeout|failed|backlog|stuck)' $base_dir/var/log/foreman/dynflow*log* 2>/dev/null | head -30"
  log "---"
  log

  postgres_log_dir="$base_dir/var/lib/pgsql/data/log"
  postgresql_conf="$base_dir/var/lib/pgsql/data/postgresql.conf"

  if [ ! -d "$postgres_log_dir" ] && [ -f "$postgresql_conf" ]; then
    log_directory_line=`grep -E "^[[:space:]]*log_directory[[:space:]]*=" "$postgresql_conf" 2>/dev/null | tail -n1`
    if [ "$log_directory_line" != "" ]; then
      log_directory_value=`echo "$log_directory_line" | cut -d= -f2- | sed -e "s/#.*$//" -e "s/[\'\"]//g" -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//'`
      if [ "$log_directory_value" != "" ]; then
        case "$log_directory_value" in
          /*)
            candidate="$base_dir$log_directory_value"
            ;;
          *)
            candidate="$base_dir/var/lib/pgsql/data/$log_directory_value"
            ;;
        esac
        if [ -d "$candidate" ]; then
          postgres_log_dir="$candidate"
        fi
      fi
    fi
  fi

  log "// postgresql fatal events"
  log "if compgen -G '$postgres_log_dir/*.log*' > /dev/null 2>&1; then $GREP_EXTENDED -n 'FATAL:|PANIC:|wraparound|remaining connection slots|checkpoint' $postgres_log_dir/*.log* 2>/dev/null | head -30; else echo 'No PostgreSQL log files found in $postgres_log_dir'; fi"
  log "---"
  log_cmd "if compgen -G '$postgres_log_dir/*.log*' > /dev/null 2>&1; then $GREP_EXTENDED -n 'FATAL:|PANIC:|wraparound|remaining connection slots|checkpoint' $postgres_log_dir/*.log* 2>/dev/null | head -30; else echo 'No PostgreSQL log files found in $postgres_log_dir'; fi"
  log "---"
  log

  log "// certificate expirations (<7 days)"
  log "if command -v openssl >/dev/null 2>&1; then now=\$(date +%s); found=0; for cert in $base_dir/etc/pki/katello/certs/*.crt $base_dir/etc/pki/katello/certs/*.pem $base_dir/etc/pki/tls/certs/*.crt $base_dir/etc/httpd/conf.d/*.crt $base_dir/etc/foreman-proxy/*.pem $base_dir/etc/foreman-proxy/*.crt $base_dir/etc/foreman-proxy/certs/*.crt $base_dir/etc/foreman-proxy/certs/*.pem; do [ -e \"\$cert\" ] || continue; end=\$(openssl x509 -noout -enddate -in \"\$cert\" 2>/dev/null | cut -d= -f2); [ \"\$end\" = \"\" ] && continue; expiry=\$(date -d \"\$end\" +%s 2>/dev/null); [ \"\$expiry\" = \"\" ] && continue; days=$(( (expiry - now) / 86400 )); if [ \$days -le 7 ]; then echo \"ALERT: \$cert expires in \$days days (\$end)\"; else printf 'OK: %s expires in %s days (%s)\n' \"\$cert\" \"\$days\" \"\$end\"; fi; found=1; done; if [ \$found -eq 0 ]; then echo 'No Katello/Proxy/HTTPD certificates found in default locations.'; fi; else echo 'openssl command not available to inspect certificates.'; fi"
  log "---"
  log_cmd "if command -v openssl >/dev/null 2>&1; then now=\$(date +%s); found=0; for cert in $base_dir/etc/pki/katello/certs/*.crt $base_dir/etc/pki/katello/certs/*.pem $base_dir/etc/pki/tls/certs/*.crt $base_dir/etc/httpd/conf.d/*.crt $base_dir/etc/foreman-proxy/*.pem $base_dir/etc/foreman-proxy/*.crt $base_dir/etc/foreman-proxy/certs/*.crt $base_dir/etc/foreman-proxy/certs/*.pem; do [ -e \"\$cert\" ] || continue; end=\$(openssl x509 -noout -enddate -in \"\$cert\" 2>/dev/null | cut -d= -f2); [ \"\$end\" = \"\" ] && continue; expiry=\$(date -d \"\$end\" +%s 2>/dev/null); [ \"\$expiry\" = \"\" ] && continue; days=$(( (expiry - now) / 86400 )); if [ \$days -le 7 ]; then echo \"ALERT: \$cert expires in \$days days (\$end)\"; else printf 'OK: %s expires in %s days (%s)\n' \"\$cert\" \"\$days\" \"\$end\"; fi; found=1; done; if [ \$found -eq 0 ]; then echo 'No Katello/Proxy/HTTPD certificates found in default locations.'; fi; else echo 'openssl command not available to inspect certificates.'; fi"
  log "---"
  log

  log "// redis pressure signals"
  log "{ [ -f $base_dir/sos_commands/redis/redis-cli_info ] && { echo '--- redis-cli info ---'; $GREP_EXTENDED -ni 'blocked_clients|maxmemory|evicted_keys|used_memory_peak' $base_dir/sos_commands/redis/redis-cli_info 2>/dev/null; }; for redis_log in $base_dir/var/log/redis/*.log* $base_dir/var/log/redis/redis.log*; do [ -e \"\$redis_log\" ] || continue; echo \"--- \$redis_log ---\"; $GREP_EXTENDED -ni 'OOM command not allowed|Out Of Memory|evict|maxmemory|blocked client|warning.*memory' \"\$redis_log\" 2>/dev/null; done; } | head -60"
  log "---"
  log_cmd "{ [ -f $base_dir/sos_commands/redis/redis-cli_info ] && { echo '--- redis-cli info ---'; $GREP_EXTENDED -ni 'blocked_clients|maxmemory|evicted_keys|used_memory_peak' $base_dir/sos_commands/redis/redis-cli_info 2>/dev/null; }; for redis_log in $base_dir/var/log/redis/*.log* $base_dir/var/log/redis/redis.log*; do [ -e \"\$redis_log\" ] || continue; echo \"--- \$redis_log ---\"; $GREP_EXTENDED -ni 'OOM command not allowed|Out Of Memory|evict|maxmemory|blocked client|warning.*memory' \"\$redis_log\" 2>/dev/null; done; } | head -60"
  log "---"
  log

  chrony_tracking="$base_dir/sos_commands/chrony/chronyc_tracking"
  log "// chrony tracking summary"
  log "if [ -f $chrony_tracking ]; then sed -n '1,40p' $chrony_tracking 2>/dev/null; elif command -v chronyc >/dev/null 2>&1; then chronyc tracking; else echo 'chronyc tracking output not available in sosreport.'; fi"
  log "---"
  log_cmd "if [ -f $chrony_tracking ]; then sed -n '1,40p' $chrony_tracking 2>/dev/null; elif command -v chronyc >/dev/null 2>&1; then chronyc tracking; else echo 'chronyc tracking output not available in sosreport.'; fi"
  log "---"
  log

  log "// pulp inode usage"
  log "pulp_df_files=\$(find $base_dir/sos_commands -maxdepth 3 -type f -name 'df_-i*' 2>/dev/null); if [ \"$pulp_df_files\" != \"\" ]; then for df_file in $pulp_df_files; do if $GREP -qi 'pulp' \"\$df_file\" 2>/dev/null; then echo \"--- \$df_file ---\"; $GREP -i 'Filesystem' \"\$df_file\" 2>/dev/null | head -1; $GREP -i 'pulp' \"\$df_file\" 2>/dev/null; fi; done; else echo 'No df -i captures found within sos_commands.'; fi"
  log "---"
  log_cmd "pulp_df_files=\$(find $base_dir/sos_commands -maxdepth 3 -type f -name 'df_-i*' 2>/dev/null); if [ \"$pulp_df_files\" != \"\" ]; then for df_file in $pulp_df_files; do if $GREP -qi 'pulp' \"\$df_file\" 2>/dev/null; then echo \"--- \$df_file ---\"; $GREP -i 'Filesystem' \"\$df_file\" 2>/dev/null | head -1; $GREP -i 'pulp' \"\$df_file\" 2>/dev/null; fi; done; else echo 'No df -i captures found within sos_commands.'; fi"
  log "---"
  log

  log "// pulp top space consumers"
  log "if [ -d $base_dir/var/lib/pulp ]; then du -x -h --max-depth=1 $base_dir/var/lib/pulp 2>/dev/null | sort -h | tail -10; else pulp_du_file=\$(find $base_dir/sos_commands -maxdepth 3 -type f -name 'du_-x_-h_--max-depth=1*var*lib*pulp*' 2>/dev/null | head -1); if [ \"$pulp_du_file\" != \"\" ]; then echo \"--- \$pulp_du_file ---\"; sort -h \"\$pulp_du_file\" | tail -10; else echo 'No du capture available for /var/lib/pulp.'; fi; fi"
  log "---"
  log_cmd "if [ -d $base_dir/var/lib/pulp ]; then du -x -h --max-depth=1 $base_dir/var/lib/pulp 2>/dev/null | sort -h | tail -10; else pulp_du_file=\$(find $base_dir/sos_commands -maxdepth 3 -type f -name 'du_-x_-h_--max-depth=1*var*lib*pulp*' 2>/dev/null | head -1); if [ \"$pulp_du_file\" != \"\" ]; then echo \"--- \$pulp_du_file ---\"; sort -h \"\$pulp_du_file\" | tail -10; else echo 'No du capture available for /var/lib/pulp.'; fi; fi"
  log "---"
  log

  log "// httpd 5xx and timeout events"
  log "{ $GREP_EXTENDED -n ' 5[0-9][0-9] ' $base_dir/var/log/httpd/*access_log* 2>/dev/null | head -20; $GREP_EXTENDED -n 'AH[0-9]+:.*(timeout|Timed out|Timeout)' $base_dir/var/log/httpd/*error_log* 2>/dev/null | head -20; }"
  log "---"
  log_cmd "{ $GREP_EXTENDED -n ' 5[0-9][0-9] ' $base_dir/var/log/httpd/*access_log* 2>/dev/null | head -20; $GREP_EXTENDED -n 'AH[0-9]+:.*(timeout|Timed out|Timeout)' $base_dir/var/log/httpd/*error_log* 2>/dev/null | head -20; }"
  log "---"
  log

  log "// foreman-proxy errors"
  log "$GREP_EXTENDED -n 'ERROR|Failed|Timeout|certificate|puppet|dhcp|tftp|dns' $base_dir/var/log/foreman-proxy/*.log* 2>/dev/null | head -30"
  log "---"
  log_cmd "$GREP_EXTENDED -n 'ERROR|Failed|Timeout|certificate|puppet|dhcp|tftp|dns' $base_dir/var/log/foreman-proxy/*.log* 2>/dev/null | head -30"
  log "---"
  log

  log "// qdrouterd link/authentication issues"
  log "$GREP_EXTENDED -n 'link.*fail|SSL|TLS|SASL|authentication|connection refused|proton|idle timeout' $base_dir/var/log/qdrouterd/*.log* 2>/dev/null | head -30"
  log "---"
  log_cmd "$GREP_EXTENDED -n 'link.*fail|SSL|TLS|SASL|authentication|connection refused|proton|idle timeout' $base_dir/var/log/qdrouterd/*.log* 2>/dev/null | head -30"
  log "---"
  log

  log "// systemd failed services"
  log "if [ -f $base_dir/sos_commands/systemd/systemctl_list-units ]; then $GREP_EXTENDED -n 'loaded.*failed' $base_dir/sos_commands/systemd/systemctl_list-units 2>/dev/null; elif [ -f $base_dir/sos_commands/systemd/systemctl_--failed ]; then cat $base_dir/sos_commands/systemd/systemctl_--failed; else echo 'No systemctl unit listings found in sosreport.'; fi"
  log "---"
  log_cmd "if [ -f $base_dir/sos_commands/systemd/systemctl_list-units ]; then $GREP_EXTENDED -n 'loaded.*failed' $base_dir/sos_commands/systemd/systemctl_list-units 2>/dev/null; elif [ -f $base_dir/sos_commands/systemd/systemctl_--failed ]; then cat $base_dir/sos_commands/systemd/systemctl_--failed; else echo 'No systemctl unit listings found in sosreport.'; fi"
  log "---"
  log

  log "// subscription-manager status"
  log "if [ -f $base_dir/sos_commands/subscription-manager/subscription-manager_status ]; then cat $base_dir/sos_commands/subscription-manager/subscription-manager_status; else echo 'subscription-manager status output not available.'; fi"
  log "---"
  log_cmd "if [ -f $base_dir/sos_commands/subscription-manager/subscription-manager_status ]; then cat $base_dir/sos_commands/subscription-manager/subscription-manager_status; else echo 'subscription-manager status output not available.'; fi"
  log "---"
  log

  log "// foreman-maintain health summary"
  log "if [ -f $base_dir/sos_commands/foreman-maintain/foreman-maintain_health_check ]; then $GREP_EXTENDED -n 'FAIL|ERROR|WARN|SKIP' $base_dir/sos_commands/foreman-maintain/foreman-maintain_health_check 2>/dev/null; else echo 'foreman-maintain health check output not captured.'; fi"
  log "---"
  log_cmd "if [ -f $base_dir/sos_commands/foreman-maintain/foreman-maintain_health_check ]; then $GREP_EXTENDED -n 'FAIL|ERROR|WARN|SKIP' $base_dir/sos_commands/foreman-maintain/foreman-maintain_health_check 2>/dev/null; else echo 'foreman-maintain health check output not captured.'; fi"
  log "---"
  log

  log "// journalctl critical messages"
  log "journal_file=\$(find $base_dir/sos_commands/logs -maxdepth 1 -type f -name 'journalctl*' 2>/dev/null | head -1); if [ \"$journal_file\" != \"\" ]; then echo \"--- \$journal_file ---\"; $GREP_EXTENDED -ni '(priority[ =:]*[0-3]|\\bEMERG\\b|\\bALERT\\b|\\bCRIT\\b|\\bERR\\b)' \"\$journal_file\" 2>/dev/null | head -40; else echo 'No journalctl captures available.'; fi"
  log "---"
  log_cmd "journal_file=\$(find $base_dir/sos_commands/logs -maxdepth 1 -type f -name 'journalctl*' 2>/dev/null | head -1); if [ \"$journal_file\" != \"\" ]; then echo \"--- \$journal_file ---\"; $GREP_EXTENDED -ni '(priority[ =:]*[0-3]|\\bEMERG\\b|\\bALERT\\b|\\bCRIT\\b|\\bERR\\b)' \"\$journal_file\" 2>/dev/null | head -40; else echo 'No journalctl captures available.'; fi"
  log "---"
  log

  log "// load average snapshot"
  log "if [ -f $base_dir/sos_commands/process/uptime ]; then cat $base_dir/sos_commands/process/uptime; elif [ -f $base_dir/uptime ]; then cat $base_dir/uptime; else echo 'uptime output not available.'; fi"
  log "---"
  log_cmd "if [ -f $base_dir/sos_commands/process/uptime ]; then cat $base_dir/sos_commands/process/uptime; elif [ -f $base_dir/uptime ]; then cat $base_dir/uptime; else echo 'uptime output not available.'; fi"
  log "---"
  log

  log "// top memory consumers"
  log "if [ -f $base_dir/sos_commands/process/ps_auxwww ]; then head -n1 $base_dir/sos_commands/process/ps_auxwww; tail -n +2 $base_dir/sos_commands/process/ps_auxwww | sort -rk4 | head -10; elif [ -f $base_dir/ps ]; then head -n1 $base_dir/ps; tail -n +2 $base_dir/ps | sort -rk4 | head -10; else echo 'ps aux output not available.'; fi"
  log "---"
  log_cmd "if [ -f $base_dir/sos_commands/process/ps_auxwww ]; then head -n1 $base_dir/sos_commands/process/ps_auxwww; tail -n +2 $base_dir/sos_commands/process/ps_auxwww | sort -rk4 | head -10; elif [ -f $base_dir/ps ]; then head -n1 $base_dir/ps; tail -n +2 $base_dir/ps | sort -rk4 | head -10; else echo 'ps aux output not available.'; fi"
  log "---"
  log
